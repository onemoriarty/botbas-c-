...
def restart_tor():
    try:
        subprocess.run(['pkill', '-9', 'tor'], check=False) # Forcefully kill tor
...
    try:
        if os.path.exists('session.cookies'):
...
                        if not stop_tor():
                            return False
                        time.sleep(random.randint(300, 600)) # Keep long sleep for backoff as website suggests rate limiting.
                        return False
...
process_item_url = "https://youtu.be/8j9bAIq3oJI?si=f7k_bB0dQ4YWN2Hm"
quantity = "25"
repeat_count = 10
